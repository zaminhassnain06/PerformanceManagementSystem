...
namespace PMS
{
    public partial class TPM_Machines : Form
    {
        ...
        private void buttonMenuTPM_Machines_Click(object sender, EventArgs e)
        {
            ButtonsUtility.Menu(0);
            this.Close();
        }

        private void buttonLogoutTPM_Machines_Click(object sender, EventArgs e)
        {
            ButtonsUtility.NavigateToLogin();
            this.Close();
        }

        private void buttonExitTPM_Machines_Click(object sender, EventArgs e)
        {
            ButtonsUtility.ExitProgram();
        }

        private void comboBoxWorkShopMachine_SelectedIndexChanged(object sender, EventArgs e)
        {
            WorkShopId = ButtonsUtility.GetWorkShopId(comboBoxWorkShopMachine.Text);
            GetMachines();
            labelSucessfulmaintenanace.Visible = false;
        ...
        {
            DataTable dt = new DataTable();
            dt = ButtonsUtility.ViewMachinesDetailsTPMMachine(WorkShopId);
            ...
            for (int i = 0; i < rows; i++)
            {
                ...
                dataGridViewTPM.Rows.Add(dt.Rows[i][0], dt.Rows[i][1], dt.Rows[i][2], dt.Rows[i][3]);

                if (dt.Rows[i][5] == "Perform Maintenance")
                {
                    dataGridViewTPM.Rows[i].Cells[4].Value = "Maintenance required";
                    dataGridViewTPM.Rows[i].Cells[4].Style.BackColor = Color.Red;
                    dataGridViewTPM.Rows[i].Cells[4].Style.ForeColor = Color.Red;


                }
                else
                {
                    dataGridViewTPM.Rows[i].Cells[4].Value = "Maintenance not required";
                    dataGridViewTPM.Rows[i].Cells[4].ReadOnly = true;
                    dataGridViewTPM.Rows[i].Cells[4].Style.BackColor = Color.Green;
                    dataGridViewTPM.Rows[i].Cells[4].Style.ForeColor = Color.Green;
                }
                dataGridViewTPM.Rows[i].Cells[5].Value = dt.Rows[i][4];
            }
        ...
        {
            int MachineId = (Int32)dataGridViewTPM.CurrentRow.Cells[5].Value; //Calculates PkId of MachineIdPk and Saves it in hidden column index 6 of grid view
           
          

          
            if (e.ColumnIndex == 4)//Perform Maintenance
            {

                int rows = ButtonsUtility.PerformTPMMachineMaintenance(MachineId);
                if (rows > 0)
                {
        ...
        private void buttonBackTPM_Machines_Click(object sender, EventArgs e)
        {
        ...
        private void TPM_Machines_Load(object sender, EventArgs e)
        {
