...
namespace PMS
{
    public partial class WorkerView : Form
    {
        ...
        private void GetGridData()
        {
            ...
            dt = ButtonsUtility.GetAllWorkers();
            int rows = dt.Rows.Count;
            if (rows > 0)
            {
             

                dataGridViewWorkerDetails.Rows.Clear();

                for (int i = 0; i < rows; i++)
                {

                    dataGridViewWorkerDetails.Rows.Add(dt.Rows[i][0], dt.Rows[i][1], dt.Rows[i][2], dt.Rows[i][3], dt.Rows[i][4], dt.Rows[i][5]);
            ...
            else
            {
                ...
                this.dataGridViewWorkerDetails.Rows.Clear();
        ...
        private void dataGridViewWorkerDetails_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {

            if (dataGridViewWorkerDetails.SelectedRows.Count == 1)
            {
                 DataGridViewRow row = this.dataGridViewWorkerDetails.SelectedRows[0];
                 workerId = row.Cells["EmpId"].Value.ToString();
            }

        }

        private void buttonEditWorkerDetails_Click(object sender, EventArgs e)
        {
           if(dataGridViewWorkerDetails.SelectedRows.Count==1)
           {
               AddNewWorker addWorker = new AddNewWorker(workerId);
               addWorker.Show();
               this.Close();
        ...
        {
            AddNewWorker addWorker = new AddNewWorker(string.Empty);
            addWorker.Show();
            this.Close();
        }
