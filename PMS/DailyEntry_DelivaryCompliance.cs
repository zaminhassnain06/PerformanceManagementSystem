...
namespace PMS
{
    public partial class DailyEntry_DelivaryCompliance : Form
    {
        private int Item_Id=0;
        int FixedEntryValueCheck = 0;
        public DailyEntry_DelivaryCompliance(int itemId)
        {
            InitializeComponent();
            Item_Id = itemId;
            if (itemId > 0)
            {
                ...
                labelItemNameDelivaryCompliance.Text = ItemName;
               // comboBoxItemsDailyEntryDelivaryCompliance.Visible = false;
               // labelSelectItemDailyEntryDelivaryCompliance.Visible = false;
                InitializeDropDownItemValues();
                TextBoxesCalculatedValuesofItemSelectedbyCombo(labelItemNameDelivaryCompliance.Text, DateTime.Today.ToString("yyyy-MM-dd"));
            }
            else if (Item_Id == 0)
            {
        ...
        {
            DelivaryComplianceKPI delComp = new DelivaryComplianceKPI(Item_Id);
            delComp.Show();
            this.Close();
        }
        ...
        private void buttonDailyEntrySave_Click(object sender, EventArgs e)
        {
            ...
            {
                labelPleaseSelectItemDelivaryCompliance.Visible = true;
            }
            else
            {
                if (Item_Id > 0)
                {
                    ...
                    {
                        labelRecordupdatedSucessfullyDailyEntryDelComp.Text = "Record entered sucessfully.";
                        labelRecordupdatedSucessfullyDailyEntryDelComp.Visible = true;
                    }
                    else
                    {
        ...
        private void dateTimePickerCustomerRejection_ValueChanged(object sender, EventArgs e)
        {
            ...
            {
                TextBoxesCalculatedValuesofItemSelectedbyCombo(comboBoxItemsDailyEntryDelivaryCompliance.Text, dateTimePickerDailyEntry.Text);
            }
            else if (comboBoxItemsDailyEntryDelivaryCompliance.Visible == false)
            {
                TextBoxesCalculatedValuesofItemSelectedbyCombo(labelItemNameDelivaryCompliance.Text, dateTimePickerDailyEntry.Text);
                labelEditErrorMessageforDateDelComp.Visible = false;

            }
            labelRecordupdatedSucessfullyDailyEntryDelComp.Visible = false;
            labelEditErrorMessageforDateDelComp.Visible = false;
        ...
        {
            TextBoxesCalculatedValuesofItemSelectedbyCombo(comboBoxItemsDailyEntryDelivaryCompliance.Text, dateTimePickerDailyEntry.Text);
            labelPleaseSelectItemDelivaryCompliance.Visible = false;
            labelRecordupdatedSucessfullyDailyEntryDelComp.Visible = false;
            labelEditErrorMessageforDateDelComp.Visible = false;
            labelAlreadySaved.Visible = false;
        ...
        private void buttonEditDailyEntry_Click(object sender, EventArgs e)
        {
            ...
            {
                labelEditErrorMessageforDateDelComp.Visible = true;
            }
            else if(Item_Id==0 && comboBoxItemsDailyEntryDelivaryCompliance.Text=="")
            {
                labelPleaseSelectItemDelivaryCompliance.Visible = true;
            ...
            
                {
                    ...
                    labelRecordupdatedSucessfullyDailyEntryDelComp.Text = "Record updated sucessfully.";
                ...
                {
                    labelRecordupdatedSucessfullyDailyEntryDelComp.Text = "Record is not already saved. Please save the record first.";
        ...
        private void textBoxAcutalDelivaryCompliance_KeyPress(object sender, KeyPressEventArgs e)
       {
            ...
            if (e.KeyChar == 13)
            {
                buttonDailyEntrySaveDelComp.PerformClick();
            }
        }
