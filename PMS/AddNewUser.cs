...
namespace PMS
{
    public partial class AddNewUser : Form
    {
        ...
        {
            InitializeComponent();
            InitializeDropDownUsersValues();
            InitializeDropDownUsersTypeValues();
        }

        private void InitializeDropDownUsersTypeValues()
        {
            DataTable dt = new DataTable();
            dt = ButtonsUtility.InitializeUserTypesDropDown();
            comboBoxUserType.Items.Clear();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                comboBoxUserType.Items.Add(dt.Rows[i][0]);

            }
        }
        private void InitializeDropDownUsersValues()
        {

            DataTable dt = new DataTable();
            dt = ButtonsUtility.InitializeUser();
            comboBoxUsers.Items.Clear();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                comboBoxUsers.Items.Add(dt.Rows[i][0]);
        ...
        {
            User_Id = ButtonsUtility.GetUserId(comboBoxUsers.Text);
        ...
        private void comboBoxUserType_SelectedIndexChanged(object sender, EventArgs e)
        {
            labelRequiredFileds.Visible = false;
            labelRecordupdatedSucessfully.Visible = false;
            User_Type_Id = ButtonsUtility.GetUserTypesId(comboBoxUserType.Text);
        }

        private void buttonSave_Click(object sender, EventArgs e)
        {
            if (textBoxUserName.Text == "" || textBoxPassword.Text == "" || comboBoxUserType.Text == "" || textBoxContactNumber.Text == "" || textBoxEmail.Text == "" || textBoxAddress.Text=="")
            {
                labelRequiredFileds.Visible = true;
            }
            else 
            {
                int rows = ButtonsUtility.SaveUserDetails(textBoxUserName.Text, textBoxPassword.Text, User_Type_Id, textBoxContactNumber.Text, textBoxEmail.Text, textBoxAddress.Text);
                if(rows>0)
                {
                ...
                {
                    labelUserNameExits.Visible = true;
        ...
        private void textBoxUserName_TextChanged(object sender, EventArgs e)
        {
            labelRequiredFileds.Visible = false;
            labelRecordupdatedSucessfully.Visible = false;
            labelUserNameExits.Visible = false;
        }
        ...
        private void buttonEdit_Click(object sender, EventArgs e)
        {
            ...
            {
                int rows = ButtonsUtility.EditUserDetails(textBoxUserName.Text, textBoxPassword.Text, User_Type_Id, textBoxContactNumber.Text, textBoxEmail.Text, textBoxAddress.Text,User_Id);
                if (rows > 0)
                {
                    labelRecordupdatedSucessfully.Visible = true;
                    InitializeDropDownUsersValues();
