...
namespace PMS
{
    public partial class CostofRework : Form
    {
        private int Item_Id = 0;
        private int Month_Id = 0;
     
        public CostofRework(int ItemId)
        {
            InitializeComponent();
            if (ItemId > 0)
            {

                labelItemNameCostOfRework.Text = ButtonsUtility.ItemNameUtility(ItemId);
                labelItemNameCostOfRework.Visible = true;
                Item_Id = ItemId;
            ...
            {
                labelItemNameCostOfRework.Visible = false;
        ...
        {
            DataTable dt = new DataTable();
            dt = ButtonsUtility.GetCalculationsReworkCostMonthly(Item_Id);
            ...
            try
            {
                ...
                dataGridCalcViewCostOfRework.Rows.Clear();

                for (int i = 0; i < rows; i++)
                {

                    dataGridCalcViewCostOfRework.Rows.Add(dt.Rows[i][0], dt.Rows[i][1], dt.Rows[i][2], dt.Rows[i][3]);


                    if (dt.Rows[i][3].ToString() == "Achieved")
                    {

                        dataGridCalcViewCostOfRework.Rows[i].Cells[3].Style.BackColor = Color.LightGreen;
                        dataGridCalcViewCostOfRework.Rows[i].Cells[3].Style.ForeColor = Color.Black;
                    ...
                    else
                    {
                        ...
                        dataGridCalcViewCostOfRework.Rows[i].Cells[3].Style.BackColor = Color.IndianRed;
                        dataGridCalcViewCostOfRework.Rows[i].Cells[3].Style.ForeColor = Color.Black;
                    }
                ...
                if (chartCostRework.Series.IndexOf("Cost of Rework Achieved %") != -1)
                {
                    chartCostRework.Series.Clear();
                }
                if (chartCostRework.Series.IndexOf("Cost of Rework Not Achieved %") != -1)
                {
                    chartCostRework.Series.Clear();
                }
                chartCostRework.Series.Add("Cost of Rework Achieved %");
                chartCostRework.Series.Add("Cost of Rework Not Achieved %");

                chartCostRework.Series["Cost of Rework Achieved %"].Color = Color.LightGreen;
                chartCostRework.Series["Cost of Rework Not Achieved %"].Color = Color.IndianRed;
                for (int i = 0; i < rows; i++)
                {
                    if (dt.Rows[i][3].ToString() == "Achieved")
                    {

                        chartCostRework.Series["Cost of Rework Achieved %"].Points.AddXY(dt.Rows[i][0].ToString(), dt.Rows[i][1]);
                        chartCostRework.Series["Cost of Rework Not Achieved %"].Points.AddXY(dt.Rows[i][0].ToString(), 0);
                        chartCostRework.Series["Cost of Rework Achieved %"].Points[i].IsValueShownAsLabel = true;
                        chartCostRework.Series["Cost of Rework Not Achieved %"].Points[i].IsValueShownAsLabel = false;
                    }
                    else
                    {
                        chartCostRework.Series["Cost of Rework Achieved %"].Points.AddXY(dt.Rows[i][0].ToString(), 0);
                        chartCostRework.Series["Cost of Rework Not Achieved %"].Points.AddXY(dt.Rows[i][0].ToString(), dt.Rows[i][1]);
                        chartCostRework.Series["Cost of Rework Achieved %"].Points[i].IsValueShownAsLabel = false;
                        chartCostRework.Series["Cost of Rework Not Achieved %"].Points[i].IsValueShownAsLabel = true;
                    }


                }
                chartCostRework.ChartAreas[0].AxisX.Interval = 1;
        ...
        private void buttonMenuCostOfRework_Click(object sender, EventArgs e)
        {
            ButtonsUtility.Menu(Item_Id);
            this.Close();
        }

        private void buttonLogoutCostOfRework_Click(object sender, EventArgs e)
        {
            ButtonsUtility.NavigateToLogin();
            this.Close();
        }

        private void buttonExitCostOfRework_Click(object sender, EventArgs e)
        {
        ...
        private void comboBoxItemsReworkRatioKPI_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            labelItemNameCostOfRework.Text = comboBoxItemsReworkRatioKPI.Text;
            labelItemNameCostOfRework.Visible = true;
            labelPleaseSelectItem.Visible = false;
            if (Item_Id > 0)
            {
                ButtonsUtility.SaveMonthsforObservationsActivitiesCostofRework(Item_Id);
            ...
            {
                decimal MonthTarget = ButtonsUtility.GetMonthTargetReworkCost(Month_Id, Item_Id,dateTimePickerCostRework.Value.Year);
        ...
        {
            int row = ButtonsUtility.InsertTargetReworkCost(Item_Id, Month_Id, Convert.ToDecimal(textBoxTarget.Text),dateTimePickerCostRework.Value.Year);
        ...
        {
            int row = ButtonsUtility.EditTargetReworkCost(Item_Id, Month_Id, Convert.ToDecimal(textBoxTarget.Text),dateTimePickerCostRework.Value.Year);
        ...
        private void GetObervationActivities()
        {
            ...
            ds = ButtonsUtility.GetActivitesObservationCostOfRework(Item_Id);
            dataGridViewCostofReworkObs.DataSource = ds.Tables[0];
            dataGridViewCostofReworkObs.Columns[1].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;

            dataGridViewAcitvityCostRework.DataSource = ds.Tables[1];
            dataGridViewAcitvityCostRework.Columns[1].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
        }

        private void dataGridViewCostofReworkObs_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            DataGridViewRow dgvObsRow = dataGridViewCostofReworkObs.CurrentRow;
            int rowindex = dataGridViewCostofReworkObs.CurrentCell.RowIndex;
            ButtonsUtility.EditCustomerCostofRework(Item_Id, dgvObsRow.Cells[1].Value == DBNull.Value ? "" : dgvObsRow.Cells[1].Value.ToString(), rowindex + 1, 0);
            GetObervationActivities(); 
        }

        private void dataGridViewAcitvityCostRework_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            DataGridViewRow dgvObsRow = dataGridViewAcitvityCostRework.CurrentRow;
            int rowindex = dataGridViewAcitvityCostRework.CurrentCell.RowIndex;
            ButtonsUtility.EditCustomerCostofRework(Item_Id, dgvObsRow.Cells[1].Value == DBNull.Value ? "" : dgvObsRow.Cells[1].Value.ToString(), rowindex + 1, 0);
            GetObervationActivities(); 
        }

        private void buttonBackCostOfRework_Click(object sender, EventArgs e)
        {
        ...
        private void dateTimePickerCostRework_ValueChanged(object sender, EventArgs e)
        {
